...
// struct for node in list of processes that share a
...
struct proc {
  ...
  int    priority;      // Scheduling priority - 0 to NQUEUES-1 
...
struct queue {  // struct for MLFQ scheduler queue
  struct spinlock lock;
  uint   timeslice;
  struct proc *head;
  struct proc *tail;
